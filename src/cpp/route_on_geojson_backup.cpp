...
int main(){
    ...
    vector<Node> nodes;
    unordered_map<long long,int> coordToId;
    auto key=[&](double lat,double lon){ long long la=llround(lat*1e5), lo=llround(lon*1e5); return (la<<32)^(lo & 0xffffffff); };
    auto getId=[&](double lat,double lon){
        long long k=key(lat,lon);
        auto it=coordToId.find(k);
    ...
    // ---------- Load cable-geo.json ----------
    ifstream gj("cable-geo.json");
    if(!gj){ cerr<<"cable-geo.json not found\n"; return 1; }
    ...
    cerr<<"graph nodes="<<nodes.size()<<" edges="<<edges<<" lines="<<countLS<<"\n";

        // ---------- Load connections ----------
        ifstream csv("region_to_region_bidirectional.csv");
    if(!csv){ cerr<<"csv not found\n"; return 1; }
    string line; getline(csv,line);
    vector<Conn> conns;
    ...
    auto nearestK=[&](double lat,double lon,int K,double maxKm){
        ...
        for(int r=0; r<=20 && (int)cand.size()<K; ++r){
            for(int di=-r; di<=r; ++di){
                for(int dj=-r; dj<=r; ++dj){
                    ...
                    for(int idx: it->second){
                        ...
                        if(d<=maxKm) cand.push_back({d,idx});
                    }
                }
            }
        }
    ...
    for(auto &kv : bySource){
        ...
        for(auto &p:sCands){ dist[p.second]=p.first*2.0; pq.push({dist[p.second],p.second}); }
        while(!pq.empty()){
        ...
        for(size_t idx: kv.second){
            ...
            for(auto &tp: tCands){ 
                double cost = dist[tp.second] + tp.first*2.0; 
            ...
            // Only use direct connection if cable route is not available or much worse
            double direct_km = hav(c.slat, c.slon, c.tlat, c.tlon);
            double direct_cost = direct_km * PENALTY; // Apply 5x penalty to direct connections
            if(bestCost > direct_cost * 2.0 && direct_km < 50000.0) {
                // Only use direct connection if cable route is significantly worse (2x the direct cost)
                if(!first) out<<",\n"; first=false;
